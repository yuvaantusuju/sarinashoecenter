...
function addHookAliases(aliases, type) {
...
const toResolveMap = (map) => Object.entries(map).map(([key, value]) => [key, require.resolve(value)]);
// Override built-in React packages if necessary
function overrideReact(config) {
...
function isApp() {
    return (process.env.__NEXT_PRIVATE_PREBUNDLED_REACT === "next" ||
        process.env.__NEXT_PRIVATE_PREBUNDLED_REACT === "experimental");
}
export function applyOverride() {
    mod._resolveFilename = ((originalResolveFilename, requestMapApp, requestMapPage, request, parent, isMain, options) => {
