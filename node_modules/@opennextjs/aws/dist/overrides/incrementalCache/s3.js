...
const { CACHE_BUCKET_REGION, CACHE_BUCKET_KEY_PREFIX, OPEN_NEXT_BUILD_ID, CACHE_BUCKET_NAME, } = process.env;
...
function buildS3Key(key, extension) {
    return path.posix.join(CACHE_BUCKET_KEY_PREFIX ?? "", extension === "fetch" ? "__fetch" : "", OPEN_NEXT_BUILD_ID ?? "", extension === "fetch" ? key : `${key}.${extension}`);
}
