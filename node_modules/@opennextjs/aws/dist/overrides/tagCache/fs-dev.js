...
const tagCache = {
    ...
    getLastModified: async (path, lastModified) => {
        ...
        const hasExpiredTag = tags.some((tagPathMapping) => {
            if (tagPathMapping.path.S === buildKey(path) &&
        ...
        const nonExpiredRevalidatedTags = tags.filter((tagPathMapping) => tagPathMapping.path.S === buildKey(path) &&
    ...
    isStale: async (path, lastModified) => {
        return tags.some((entry) => {
            if (entry.path.S !== buildKey(path))
                return false;
            if (!entry.stale?.N)
                return false;
            ...
            return (Number.parseInt(entry.revalidatedAt.N) > (lastModified ?? 0) &&
