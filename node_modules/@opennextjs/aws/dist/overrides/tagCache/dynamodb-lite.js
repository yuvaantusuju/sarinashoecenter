...
function buildDynamoObject(path, tags, revalidatedAt, stale, expire) {
    const obj = {
...
const tagCache = {
    ...
    async getByPath(path) {
        try {
            ...
            const cache = store?.requestCache.getOrCreate("dynamoDb:getByPath");
            if (cache?.has(path)) {
            ...
            const resultTags = tags.map((tag) => tag.replace(`${OPEN_NEXT_BUILD_ID}/`, ""));
    ...
    async getByTag(tag) {
        try {
            ...
            const { CACHE_DYNAMO_TABLE, OPEN_NEXT_BUILD_ID } = process.env;
            const store = globalThis.__openNextAls.getStore();
            const cache = store?.requestCache.getOrCreate("dynamoDb:getByTag");
            if (cache?.has(tag)) {
            ...
            const { Items } = (await result.json());
            // We need to remove the buildId from the path
            const paths = Items?.map(({ path: { S: key } }) => key?.replace(`${OPEN_NEXT_BUILD_ID}/`, "") ?? "") ?? [];
            cache?.set(tag, paths);
            return paths;
        }
        catch (e) {
    ...
    async getLastModified(key, lastModified) {
        try {
            ...
            const store = globalThis.__openNextAls.getStore();
            const cache = store?.requestCache.getOrCreate("dynamoDb:getLastModified");
    ...
    async isStale(key, lastModified) {
        try {
            ...
            const { CACHE_DYNAMO_TABLE } = process.env;
            const store = globalThis.__openNextAls.getStore();
            const itemsCache = store?.requestCache.getOrCreate("dynamoDb:revalidateQueryItems");
            const cacheKey = `${key}:${lastModified ?? 0}`;
            let items;
