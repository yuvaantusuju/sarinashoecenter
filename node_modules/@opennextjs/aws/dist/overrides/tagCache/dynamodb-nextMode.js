...
const awsFetch = (body, type = "query") => {
    const { CACHE_BUCKET_REGION } = process.env;
...
function buildDynamoObject(tag, revalidatedAt, stale, expire) {
    const obj = {
 ...
 */
function checkItemsCache(tags, itemsCache, compute) {
    const uncachedTags = [];
    let hasMatch = false;
    for (const tag of tags) {
        if (itemsCache?.has(tag)) {
            if (compute(itemsCache.get(tag)))
                hasMatch = true;
        }
        else {
 ...
 */
async function fetchAndCacheItems(uncachedTags, itemsCache, compute) {
    const { CACHE_DYNAMO_TABLE } = process.env;
    const response = await awsFetch(JSON.stringify({
        RequestItems: {
            [CACHE_DYNAMO_TABLE ?? ""]: {
    ...
    const responseItems = Responses?.[CACHE_DYNAMO_TABLE ?? ""] ?? [];
...
export default {
    ...
    hasBeenRevalidated: async (tags, lastModified) => {
        ...
        if (tags.length > 100) {
            throw new RecoverableError("Cannot query more than 100 tags at once. You should not be using this tagCache implementation for this amount of tags");
        }
        const store = globalThis.__openNextAls.getStore();
        const itemsCache = store?.requestCache.getOrCreate("ddb-nextMode:tagItems");
        const now = Date.now();
        const compute = (item) => {
            if (!item)
                return false;
            if (item.expire?.N) {
                ...
                if (expiry <= now && expiry > (lastModified ?? 0))
                    return true;
            }
    ...
    isStale: async (tags, lastModified) => {
        if (globalThis.openNextConfig.dangerous?.disableTagCache) {
            return false;
        }
        if (tags.length === 0)
        ...
        const compute = (item) => {
            ...
            return (revalidatedAt > (lastModified ?? 0) &&
