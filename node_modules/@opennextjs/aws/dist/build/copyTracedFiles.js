...
const nonLinuxPlatformRegex = getCrossPlatformPathRegex(`/node_modules/(?:@[^/]+/)?(?:[^/]+-)?(${platformPattern})-[^/]+/`, { escape: false });
...
--------------------------------------------------------------------------------`);
    ...
    const skippedPlatformPackages = new Set();
    filesToCopy.forEach((to, from) => {
        ...
        if (!process.env.OPEN_NEXT_SKIP_PLATFORM_FILTER &&
            isNonLinuxPlatformPackage(from)) {
            const match = from.match(/node_modules\/(?:\.pnpm\/.*\/node_modules\/)?((?:@[^/]+\/)?[^/]+)/);
            if (match) {
    ...
    if (skippedPlatformPackages.size > 0) {
        logger.debug("Skipped non-Linux platform packages:", [...skippedPlatformPackages].join(", "));
    }
