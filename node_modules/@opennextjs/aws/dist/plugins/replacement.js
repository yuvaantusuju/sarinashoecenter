 ...
 * @returns
 */
export function openNextReplacementPlugin({ target, replacements, deletes, name, }) {
    return {
        name: name ?? "opennext",
        setup(build) {
                ...
                await Promise.all([
                    ...
                    ...(replacements ?? []).map(async (filename) => {
                        ...
                        for (const match of matches) {
                            ...
                            const id = match[1];
                            const pattern = new RegExp(`\/\/#override (${id})\n([\\s\\S]*?)\/\/#endOverride`, "g");
                            logger.debug(chalk.blue(`Open-next replacement plugin ${name}`), `Apply override ${id} from ${filename}`);
